...
if ($_GET['key'] && $_GET['token']) {
    ...
    $select_users = $conn->prepare("SELECT * FROM Admin WHERE email_id = ? and token = ?");
...
<?php
    ...
    if(isset($_POST["token"]) && $_POST["pass"] && $_POST["email"]){
      ...
      $curDate = date("Y-m-d H:i:s");
        ...
        $sql = "SELECT * FROM Admin WHERE email_id = ? AND token = ?";
        ...
        if($pass=$cpass){
         ...
         $change_pass_query = $conn->prepare("UPDATE Admin SET password =?, token= ?,exp_date=? WHERE email_id=? ");
